...
> _미래를 예측하는 가장 훌륭한 방법은 바로 직접 미래를 만드는 것이다._  
...
|기호로 열 구분, -기호로 행 구분을 하여 표를 만든다.  
표 안에서도 텍스트 서식 사용이 된다.
...
각주를 달고 싶은 위치에 `[^각주이름]`을 입력하고, 각주에 대한 설명은 `[^각주이름]:`코드 옆에 입력하면 위 첨자 각주가 생기고 각주 설명이 문서 하단에 생긴다.

10. ### **기타**

1) 줄 바꾸기 : 줄 바꾸기를 하고 싶은 곳에서 두 칸 공백을 넣어주고 엔터!

2. 숫자 리스트 순서 끊기 : 순서 끊고 싶은 부분 부터 숫자 뒤에 점 대신 다른 기호 사용하면 그 시점부터 다시 1번 시작. 그 다음 항목부터 숫자 뒤에 점 찍으면 숫자가 다시 이어짐...
3. VScode에 내장된 Markdown Preview[^프리뷰]에서는 확인 되지 않는 서식들이 있어서 git add > commit > push 하고 gitHub 가서 일일이 확인함...
   근데 쌓이는 로그도 많고 비효율적이라 확장툴 설치 : **_Markdown Preview Enhanced_** > 탭 오른쪽 상단 문서랑 돋보기 있는 아이콘 눌러 실행 시키면 브라우저에서 어떻게 보일지 사이드바로 바로 확인 가능!
   [^프리뷰]: (ctrl + shift + p) 또는 (.md 탭 우클릭 Open Preview)

4. 콜아웃[^콜아웃]: 강조하고 싶은 내용을 박스 안에 넣는 기능. \> 기호를 앞에 적고 여러 줄로 입력할 수 있다.
   [^콜아웃]: `>[!note], >[!abstract], >[!info], >[todo!], >[tip!], >[success!]` 등 다양한 콜아웃이 있다.

> [!note] 아이콘이 들어와야 하는데...
> 찾아보니 렌더링 오류가 좀 있는 듯 하다

`> [!note] 아이콘이 들어와야 하는데...  `
`> 찾아보니 렌더링 오류가 좀 있는 듯 하다`

4. \`(백틱) 사용하지 않고 특수기호 쓰는 법 : \\ (역슬래쉬 : 엔터키 위에 있는 기호키)를 기호 앞에 쓰면 코드 블록 쓸 필요 없이 특수 기호 사용가능하다.[^기호사용]
   [^기호사용]: 이 방법을 늦게 알아서 모든 기호를 블록 인용구 만드는 방식처럼 백틱으로 감싸서 작업했다... 기가 막혀🫠

5. 링크 삽입 : `[링크명](링크주소)` [▶️김효경 과제 깃허브](https://github.com/cactus30301-bit/homework.git)[^깃주소]
   이미지 링크 삽입 : `[![링크명](이미지 경로/URL)](링크주소)`
   [^깃주소2][![김효경 과제 깃허브](/src/md/images/door.jpg)](https://github.com/cactus30301-bit/homework.git)
